...
export class EventsService {
  ...
  public addEvent(event: IEvent): Observable<boolean> {
    ...
    return this.httpClient.post<boolean>(`https://localhost:7067/api/events/add`, {...event, creator}, { headers });
  }

  public editEvent(event: IEvent): Observable<boolean> {
    const headers = this.buildRequestHeaders();
    const creator = this.userService.getUser();
    return this.httpClient.post<boolean>(`https://localhost:7067/api/events/${event.id}/edit`, {...event, creator}, { headers });
  }
